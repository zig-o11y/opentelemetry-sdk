...
pub fn build(b: *std.Build) void {
    ...
    const benchmark_args = b.option([]const []const u8, "benchmark-args", "Arguments to pass to benchmark runs");
    ...
    const metrics_benchmarks = buildBenchmarks(b, "benchmarks/metrics", sdk_lib.root_module, benchmark_mod, benchmark_args) catch |err| {
        std.debug.print("Error building metrics benchmarks: {}\n", .{err});
        return;
    };
    defer b.allocator.free(metrics_benchmarks);
    for (metrics_benchmarks) |tuple| {
        benchmarks_step.dependOn(&tuple.run_step.step);
    }
}
...
fn buildBenchmarks(
    ...
    benchmark_args: ?[]const []const u8,
) ![]struct {
    exe: *std.Build.Step.Compile,
    run_step: *std.Build.Step.Run,
} {
    var bench_tests = std.ArrayList(struct {
        exe: *std.Build.Step.Compile,
        run_step: *std.Build.Step.Run,
    }).init(b.allocator);
    ...
    while (try iter.next()) |file| {
        ...
        if (benchmark_args) |args| {
            run_step.addArgs(args);
        }

        try bench_tests.append(.{ .exe = benchmark, .run_step = run_step });
    }
