        ...
        { // run protoc
            ...
            if (try ensureProtocBinaryDownloaded(b)) |protoc_path| {
                ...
                if (!dirExists(absolute_dest_dir)) {
                    try std.Io.Dir.cwd().createDirPath(std.Options.debug_io, absolute_dest_dir);
                }
                ...
                switch (result.term) {
                    .exited => |code| if (code != 0) return StepErr.FailedToConvertProtobuf,
                    else => return StepErr.FailedToConvertProtobuf,
                }
            }
        }
